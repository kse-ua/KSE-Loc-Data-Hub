# this is the test comment for pull requests testing
...
# open log
if( interactive() ) { # would be FALSE if running from scheduler
  sink_log <- FALSE
} else {
...
ds_rail  <- tibble::tribble(
  ...
  # Import
  # "run_r_render"      , "manipulation/0-import.R"
  
  # Ellis lane
  # # 
  # ,"run_r_render"      , "manipulation/ellis-rada-hromada.R"
  ,"run_r_render"      , "manipulation/ellis-budget-2020.R"
  # ,"run_r_render"      , "manipulation/ellis-rada-hromada.R"


)

run_r_source <- function( minion ) {
  message("\nStarting `", basename(minion), "` at ", Sys.time(), ".")
  base::source(minion, local=new.env())
  # local=new.env() -- run it as it it's completely independent session
  message("Completed `", basename(minion), "`.")
  return( TRUE )
}

run_r_render <- function( minion ) {
  message("\nStarting `", basename(minion), "` at ", Sys.time(), ".")
  # base::source(minion, local=new.env())
  clear_memory <- FALSE
  rmarkdown::render(

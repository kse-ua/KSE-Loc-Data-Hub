...
```{r}
d <- 
  ds1_prep %>% select(hromada_code, starts_with("prep_score")) %>% 
  left_join(ds0 %>% select(hromada_code
...
g <- 
  d %>% 
  mutate(
    hromada_code = hromada_code %>% factor() %>% fct_reorder(prep_score)
  ) %>% 
  # sample_n(10) %>% 
  slice(1:10) %>% 
...
g %>% quick_save("prep-change",w=5,h=9)
```


```{r}
#  Large faceted graph to see correlations with other variables
# x = prep_score
# y = 
...
g <- 
  ...
  facet_wrap(facets = "item_name", scales = "free_y")+
  geom_point(shape=21,color = "black", size =3, alpha = .5, position=position_jitter(seed=42))
g %>% quick_save("prep_score-vs-other_items",w=12, h=8)

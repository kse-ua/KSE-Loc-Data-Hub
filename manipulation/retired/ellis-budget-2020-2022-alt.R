...
ds_meta_ds5_long <- 
  tibble::tibble(
    "variable_name"   = variable_labels_ds5_long %>% names()
    ,"variable_label" = variable_labels_ds5_long %>% as.character()
  )
...
# openxlsx::write.xlsx(dataset_names_dis, './data-public/derived/hromada_budget_2020_2022.xlsx')
# openxlsx::write.xlsx(dataset_names, './data-public/derived/hromada_budget_2020_2022.xlsx')

list(
  "taxes"   = dataset_names_dis
  ,"budget" = dataset_names
) %>% 
  readr::write_rds("./data-private/derived/ellis-budget-alt.rds") 

...
# 
count_occurrences <- function(data, column) {
  result <- data %>%
    count({{ column }}) %>% 
    arrange(desc(n))
...
t1 <- count_occurrences(fin, status)
t2 <- count_occurrences(fin, type)
t3 <- count_occurrences(fin, property_type)
t4 <- count_occurrences(fin, is_core)
t5 <- count_occurrences(fin, is_rural)
t6 <- count_occurrences(fin, is_mountain)
t7 <- count_occurrences(fin, is_internat)

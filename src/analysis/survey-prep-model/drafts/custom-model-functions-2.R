run_simple_scan <- function(
  ...
  # model %>% glance()
  # model %>% summary()
  # model_reduced %>% glance()
  # 
  # # variance explained by each model
  # get_model_fit(model)
  # get_model_fit(model_reduced)

  # m <- model_reduced
  # m <- model
  # # Sanity check
  # # The total misfit of the model 
  # (chisq <- with(m, null.deviance - deviance))
  ...
  (rsq_full    <- get_model_fit(model)$rsquare)
  (rsq_reduced <- get_model_fit(model_reduced)$rsquare)
  
  (pval_full    <- get_model_fit(model)$pval)
  (pval_reduced <- get_model_fit(model_reduced)$pval)
  ...
  d_out <- 
    ...
    ,"pval_reduceed"      = pval_reduced # significance of the REDUCED model

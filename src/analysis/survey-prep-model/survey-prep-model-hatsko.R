...
ds_weights <- readr::read_delim("./data-private/derived/index_preparedness_weights.csv", delim = ";")
...
ds_prep_new <- ds0 %>%
  ...
  mutate(prep_score_feb = prep_first_aid_water_feb*7.95 + prep_first_aid_fuel_feb*7.90 +
         ...
         prep_score_feb_new = (prep_first_aid_water_feb + prep_first_aid_fuel_feb +
           prep_reaction_plan_feb + prep_evacuation_plan_feb+ 
           prep_reaction_plan_oth_hromadas_feb + prep_reaction_plan_oda_feb + 
           prep_dftg_creation_feb + prep_national_resistance_feb + 
           prep_starosta_meeting_feb + prep_communal_meetiing_feb + 
           prep_online_map_feb + prep_shelter_list_feb + 
           prep_notification_check_feb + prep_backup_feb) / 14,
         
         prep_score_oct_new = (prep_first_aid_water_oct + prep_first_aid_fuel_oct +
           prep_reaction_plan_oct + prep_evacuation_plan_oct + 
           prep_reaction_plan_oth_hromadas_oct + prep_reaction_plan_oda_oct + 
           prep_dftg_creation_oct + prep_national_resistance_oct + 
           prep_starosta_meeting_oct + prep_communal_meetiing_oct + 
           prep_online_map_oct + prep_shelter_list_oct + 
           prep_notification_check_oct + prep_backup_oct)/14,
         
         prep_score_feb_new_scaled = scales::rescale(prep_score_feb_new),
         
         prep_score_oct_new_scaled = scales::rescale(prep_score_oct_new),
         
         type_new = ifelse(type == 'міська', 'міська', "сільська"))

d <- ds_prep_new %>% 
  filter(occupation!="occupied_now") %>% 
  group_by(type_new) %>%
  summarize(mean = mean(prep_score_oct_new), sd = sd(prep_score_oct_new), 
            min = min(prep_score_oct_new), max = max(prep_score_oct_new), n = n())
...
calculate_shares <- function(data, variable) {
  data %>%
    mutate(
      category = case_when(
        .data[[variable]] < 0.25 ~ "< 0.25",
        .data[[variable]] >= 0.25 & .data[[variable]] <= 0.75 ~ "0.25 - 0.75",
        .data[[variable]] > 0.75 ~ "> 0.75"
      )
    ) %>%
    count(category) %>%
...
shares_oct <- calculate_shares(ds_prep_new %>% filter(occupation!="occupied_now"), "prep_score_oct_new_scaled")
shares_feb <- calculate_shares(ds_prep_new%>% filter(occupation!="occupied_now"), "prep_score_feb_new_scaled")

ds_prep_new %>% 
  filter(occupation!="occupied_now") %>% 
  summarize(mean_oct = mean(prep_score_oct_new),
            mean_feb = mean(prep_score_feb_new))

...
g <- d %>%
  filter(occupation_fct == "Deoccupied communities (n = 16)") %>%
  ggplot(aes(y=prop, x = `Share of Housing Damaged`))+
  geom_col(fill = "red4")+
  geom_text(aes(label = pct), hjust = .3, vjust = -.2, position = position_dodge(width = .9))+
  scale_y_continuous(labels = scales::percent_format(),expand = expansion(add = c(0,.1)))+
  ggplot2::scale_fill_viridis_d(begin = 0, end = .8, direction = -1, option = "plasma",guide= guide_legend(reverse=T))+
...
g
g %>% quick_save("6-damages-deoc",w=12,h=5)

ds0 %>% filter(!is.na(occupation)) %>%
  filter(deoccupied_at_feb_2023 == 1) %>%
  select(reconstruction_financing, reconstruction_financing_text) %>%
  count(reconstruction_financing) %>% write.excel()
...
d <- ds0 %>%
  filter(!is.na(occupation)) %>%
  select(hromada_name, all_of(skills)) %>% 
  pivot_longer(
    -c(hromada_name)
    ,names_to = "action"
    ,values_to = "response"
...
g <- d %>%
  ...
  labs(title = 'What skills do executives need to address current \ncommunity challenges?', 
       fill = NULL) +
  scale_y_continuous(labels = scales::percent, limits = c(0, 0.9)) +
  scale_x_discrete(labels=c('skills_needed/fundraising' = "Fundraising \n(attracting additional funds \nto the community through \ngrants, investment projects)",
                            'skills_needed/project_management' = "Project management", 
                            'skills_needed/longterm_planning' = "Long-term planning", 
                            'skills_needed/crisis_planning' = "Crisis planning \nand management", 
                            'skills_needed/data_analysis' = "Collection and analysis \nof data for decision-making", 
                            "skills_needed/human_resourse" = "Lack of human resources", 
                            "skills_needed/other" = "Other")
                   )
...
g 
g %>%  quick_save("skills-general",w=6, h=4)
...
d2 <- 
  ds1_prep_binary_factors_feb %>% 
  pivot_longer(cols = preparation, names_to = "item_name") %>% 
  group_by(item_name,deoccupied_at_feb_2023, value, .drop = F) %>% 
  select(-c(starts_with("prep"))) %>%
  summarize(
    count = n_distinct(hromada_code)
    ,.groups = "drop"
    ,.
  ) %>% 
  group_by(deoccupied_at_feb_2023, item_name, .drop = F) %>% 
  mutate(
    prop = count/sum(count, rm.na = F)
    ,pct = scales::percent(prop, accuracy = 1)
  ) %>% 
  ungroup() %>%
  filter(value == "No")
...
d_in <- 
  d12 %>% 
    ...
    ,display_name = factor(
      ...
      ,levels =  d12 %>%
        left_join(d_meta_prep) %>%
        filter(value == "No" & deoccupied_at_feb_2023 == 1) %>%
        # filter(value == "Before Feb 24") %>%
        arrange(prop) %>%
        pull(label_en)
      )
...
g <- 
  d_in %>% 
  filter(deoccupied_at_feb_2023 == 1) %>%
  ggplot(aes(x=prop, y = fct_reorder(str_remove(display_name, "\\([^)]*\\)"), prop))) +
  geom_col(alpha = .7, fill = "red3")+
  geom_text(aes(label = str_remove(pct,"\\%"))
            ,hjust = -0.2
            ,vjust = .3
            , size = 3.2
  ...
  #   )+
  labs(
    title = "What measures had not been implemented as of October in the de-occupied hromadas"
    ,x = "Percent of respondents", y = NULL, fill = 'Deoccupied'
  )+
  theme(
    # panel.grid = element_blank()
    panel.grid.major.y  = element_blank()
...
g
g %>% quick_save("2-preparation-summary-no",w=12,h=5)
```
...
d <- ds0 %>% select(hromada_code, deoccupied_at_feb_2023, 
                    starts_with('idp_help/'), -ends_with('number')) %>%
  filter(deoccupied_at_feb_2023 == 1) %>%
  pivot_longer(-c(hromada_code, deoccupied_at_feb_2023), names_to = 'help', values_to = 'count') %>%
  mutate(count = factor(count, levels = c(0, 1))) %>%
  count(help, count, .drop = FALSE) %>%
  filter(!is.na(count)) %>%
  group_by(help, .drop = FALSE) %>% 
  mutate(freq = n/sum(n),
         help = str_remove(help, 'idp_help/')) %>%
  filter(count == 1)

p <- d %>% 
  ggplot(aes(y = fct_reorder(factor(help), freq, .desc = F), x = freq)) +
  geom_col(fill = "dodgerblue") +
  geom_label(aes(label = scales::percent(freq)))  + 
  theme_bw() +
  theme(axis.title.y = element_blank(),
  ...
  scale_y_discrete(labels=c('communal_placement' = "Placement of IDPs in \ncommunal or state-owned \npremises",
                            'private_placement' = "Placement of IDPs in \nprivate premises", 
...
p %>% quick_save("3-idp-help-deoc",w=12,h=5)

...
```{r representation-war, class.source="fold-hide"}

ds_survey %>% 
  group_by(war_zone_20_06_2022) %>% 
  summarize(
    hromada_count = n_distinct(hromada_code)
    ,.groups = "drop"
  ) %>%
  ...
  arrange(war_zone_20_06_2022, desc(hromada_survey_prop)) %>% 
  select(-c(hromada_survey_prop, hromada_total_prop)) %>%
  relocate(hromada_count_total, .after = hromada_count) %>%
  ungroup() %>%
  # neat_DT()
  gt::gt() %>%
  gt::cols_label(war_zone_20_06_2022 = 'War zone',
                 hromada_count_total = 'Total number \nof ATCs',
                 hromada_count = 'ATCs in the survey',
                 hromada_total_pct = 'Proportion of Region \nin the General Population',
                 hromada_survey_pct = 'Proportion of Region \nin the Survey'
...
```{r preparation-table, fig.height=5, fig.width=12,class.source="fold-hide", cache=T}
...
```{r prep-item-total, fig.height=4, fig.width=8,class.source="fold-hide", cache=T, eval=F}
...
```{r prep-item-total-new, fig.height=4, fig.width=8,class.source="fold-hide", cache=T}
...
```{r dftg-region-table, fig.height=3, fig.width=9,class.source="fold-hide"}
...
```{r help-military-table, fig.height=5, fig.width=9,class.source="fold-hide", eval = TRUE}
